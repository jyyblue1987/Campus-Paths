...
public final class RatPoly {
    ...
    public RatTerm getTerm(int deg) {
        //{inv: i != terms.size() and terms.get(i).getExpt() == deg}
    ...
    public boolean isNaN() {
        //{inv: i != terms.size() and terms.get(i).isNaN()}
        for(int i = 0; i < terms.size(); i++) {
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        if(!lst.isEmpty()) {
            //{inv: lst = lst[0](degree) + lst[1](degree) + ... + lst[i-1](degree) + ... + lst[lst.size() - 1]
            //where lst[j](scalar) is the coeff at index j of the list * scalar}
    ...
    private static void incremExpt(List<RatTerm> lst, int degree) {
        if(!lst.isEmpty()) {
            //{inv: lst = lst[0](degree) + lst[1](degree) + ... + lst[i-1](degree) + ... + lst[lst.size() - 1]
            //where lst[j](degree) is the exponent at index j of the list + degree}
            for(int i = 0; i < lst.size(); i++) {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        ...
        //{inv: i != terms.size() and lst.get(i).getExpt()}
        while(i < lst.size() && exp < lst.get(i).getExpt()) {
            i++;
        }
        if(i == lst.size()) {
            lst.add(newTerm);
        }
        else if (exp == lst.get(i).getExpt()) {
    ...
    /**
     * Copy operation.
     * @return a copy of all the RatTerms in RatPoly
     */
    private ArrayList<RatTerm> copy() {
        ArrayList<RatTerm> c = new ArrayList<RatTerm>();
        //{inv: i != terms.size and c = RatTerm in RatPoly}
        for(int i = 0; i < terms.size(); i++) {
            c.add(new RatTerm(terms.get(i).getCoeff(), terms.get(i).getExpt()));
        }
        return c;
    }
    ...
    public RatPoly mul(RatPoly p) {
        if(this.isNaN() || p.isNaN()) {
            ...
            for(int i = 0; i < terms.size(); i++) {
                ArrayList<RatTerm> temp = p.copy();
                scaleCoeff(temp, terms.get(i).getCoeff());
                incremExpt(temp, terms.get(i).getExpt());
    ...
    public RatPoly div(RatPoly p) {
        if(p.equals(ZERO) || this.isNaN() || p.isNaN()) {
            ...
            RatPoly result = new RatPoly();
            RatPoly remainder = new RatPoly(copy());
            //{inv: p = r * q + temp}
            while(!remainder.terms.isEmpty() && remainder.degree() >= p.degree()) {
                RatNum ce = remainder.terms.get(0).getCoeff().div(p.terms.get(0).getCoeff());
                RatPoly q = new RatPoly(new RatTerm(ce, remainder.degree() - p.degree()));
                result = result.add(q);
                remainder = remainder.sub(q.mul(p));
            }
            return result;
        }
    }
    ...
    public RatPoly differentiate() {
        if(this.isNaN()) {
            ...
            for(int i = 0; i < terms.size(); i++) {
                if((!terms.get(i).getCoeff().equals(RatNum.ZERO)) && (terms.get(i).getExpt() != 0)) {
                    sortedInsert(temp, terms.get(i).differentiate());
                }
            }
            return new RatPoly(temp);
        }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        if(this.isNaN() || integrationConstant.isNaN()) {
            ...
            ArrayList<RatTerm> temp = new ArrayList<RatTerm>();
            for(int i = 0; i < terms.size(); i++) {
                sortedInsert(temp, terms.get(i).antiDifferentiate());
            }
