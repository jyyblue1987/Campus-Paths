 ...
 |-- div5tailnum: integer (nullable = true)
   */
    ...
    public static JavaRDD<Row> QA(SparkSession spark, String dataFile) {
        // You can change this line if you wish.
        Dataset<Row> d = spark.read().parquet(dataFile);
    ...
    public static JavaRDD<Row> QB(SparkSession spark, String dataFile) {
        ...
        JavaRDD<Row> nonCancelled = d.filter(row -> row.getInt(CANCELLED) == 0);

        JavaPairRDD<Tuple2<String,Integer>, Integer> pair = nonCancelled.mapToPair(r -> new Tuple2<>(new Tuple2<>(r.get(ORIGIN_CITY_NAME).toString(), r.getInt(MONTH)), 1));

        JavaPairRDD<Tuple2<String, Integer>, Integer> result = pair.reduceByKey((a, b) -> a + b);

        JavaRDD<Row> output = result.map(tuple -> RowFactory.create(tuple._1(),tuple._2()));

        //output.take(20).forEach(line -> System.out.println(line));
    ...
    public static JavaRDD<Row> QC(SparkSession spark, String dataFile) {
        ...
        JavaPairRDD<String, Tuple2<Integer, Integer>> pairing = flights.mapToPair(r -> new Tuple2<>(r.get(ORIGIN_CITY_NAME).toString(), new Tuple2<>(r.getInt(DEP_DELAY), 1)));

        JavaPairRDD<String, Tuple2<Integer, Integer>> total = pairing.reduceByKey((a, b) -> new Tuple2<>(a._1() + b._1(), a._2() + b._2()));

        JavaPairRDD<String, Double> average = total.mapToPair(tuple -> new Tuple2<>(tuple._1(), tuple._2()._1() * 1.0 / tuple._2()._2()));

        JavaRDD<Row> output = average.map(tuple -> RowFactory.create(tuple._1(),tuple._2()));
